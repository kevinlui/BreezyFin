...
namespace Breezy.Fin
{
    ...
    /// </summary>
    public class IBClient : IDisposable, IBroker
    {
        # region IBroker interface implementation
        ...
        public void Connect()
        {
            ...
            {
                // already connected?
                string host = checkConnected("localhost");
                if (host == null)
                {
                    return;
                }

                // IBCLient_Mock does not need to create TcpClient socket
                TcpClient socket = new TcpClient(host, 7496);
                connect(socket, 0);
                ...
                case IncomingMessage.OrderStatus:
                    {
                        ...
                        OrderStatus status = (string.IsNullOrEmpty(orderstat) ? Breezy.Fin.OrderStatus.None :
                ...
                case IncomingMessage.OpenOrder:
                    {
                        ...
                        if (version >= 16)
                        {
                            ...
                            string ost = ReadStr();
                            orderState.Status = (string.IsNullOrEmpty(ost) ? Breezy.Fin.OrderStatus.None : (OrderStatus)EnumDescConverter.GetEnumValue(typeof(OrderStatus), ost));
